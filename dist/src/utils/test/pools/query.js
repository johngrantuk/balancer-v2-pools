...
const batchSwap = async (vault, poolId, swapType, tokens, tokenInSymbol, tokenOutSymbol, amount) => {
    ...
    const [tokenInDelta, tokenOutDelta] = await vault.callStatic.queryBatchSwap(swapType, [
        {
            poolId,
            assetInIndex: 0,
            assetOutIndex: 1,
...
const join = async (helpers, poolId, tokens, userData) => {
    ...
    return helpers.callStatic.queryJoin(poolId, ethers_1.constants.AddressZero, ethers_1.constants.AddressZero, {
...
const exit = async (helpers, poolId, tokens, userData) => {
    ...
    return helpers.callStatic.queryExit(poolId, ethers_1.constants.AddressZero, ethers_1.constants.AddressZero, {
        assets: tokens.map((t) => t.address),
        minAmountsOut,
        toInternalBalance: false,
        userData,
